(() => {
	...
	function sourceFor(quality) {
		return `${base}assets/video/mp4/stereo-sax-promo-${quality}.mp4`;
	}
	...
	function initCarousel(carousel, { loop = true, autoplay = true } = {}) {
		...
		const dotsRoot = carousel.parentElement?.querySelector(`[data-carousel-dots="${carousel.dataset.carousel}"]`)
	...
	clipVideos.forEach((clip) => {
		clip.addEventListener('play', () => {
			clipVideos.forEach((other) => { if (other !== clip) other.pause(); });
		});
	});
